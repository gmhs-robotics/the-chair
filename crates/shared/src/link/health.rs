...
impl HealthResponse {
    ...
    pub fn battery_ready_to_start(&self) -> bool {
        !self.initializing
            && !self.has_fault()
...
impl MotorHealth {
    ...
    pub fn is_hot(self) -> bool {
        self.temperature >= MOTOR_WARN_C || self.faults & MotorFaults::OVER_TEMPERATURE.bits() != 0
    }
    pub fn fault(self) -> Option<StopReason> {
            ...
            || self.faults & !NONFATAL_MOTOR_WARNINGS != 0
        {
            ...
            // VEXos enforces the configured current limit. Known current and heat bits remain
            // diagnostic; heat also reduces voltage locally. Unknown bits are faults.
            None
        }
    }
...
mod tests {
    ...
    fn healthy_data_can_start() {
        assert!(healthy().battery_ready_to_start());
    }
    #[test]
    ...
    #[test]
    fn temperature_warns_without_latching_on_each_motor() {
        for i in 0..4 {
            let mut h = healthy();
            h.motors[i].as_mut().unwrap().temperature = 50.0;
            assert_eq!(h.fault(), None);
            assert!(h.motors[i].unwrap().is_hot());
        }
    }
    ...
    fn driver_and_current_limit_bits_are_warnings() {
        ...
        assert_eq!(h.fault(), None);
        assert!(h.battery_ready_to_start());
    ...
    #[test]
    fn motor_over_temperature_bit_reports_heat_warning() {
        let mut h = healthy();
        h.motors[0].as_mut().unwrap().faults = MotorFaults::OVER_TEMPERATURE.bits();
        assert_eq!(h.fault(), None);
        assert!(h.motors[0].unwrap().is_hot());
    }
    #[test]
    ...
    #[test]
    fn hot_motor_does_not_block_ready_state() {
        let mut h = healthy();
        h.motors[0].as_mut().unwrap().temperature = MOTOR_WARN_C + 1.0;
        assert!(!h.has_fault());
        assert!(h.battery_ready_to_start());
        h = healthy();
