...
2. Confirm ADI C is empty. Tap CENTER with P17 at its neutral detent, then rotate it slowly both ways. Verify −8°…8° maps to 0%, intended forward travel reaches +270°/+100%, and intended reverse reaches −270°/−100%. Adjust measured constants if the installed mechanism differs; never widen fault limits merely to hide bad geometry.
3. With power off, mark physical steering center and verify the encoder-to-wheel gear ratio and safe angular range. Enter those values in `geometry.rs`. Current settings are 1:1 and ±70° with a blue cartridge. Verify a physical right turn moves the HUD wheel right and produces a positive steering value. Motion beyond the nominal range clamps the steering input instead of latching a fault.
...
7. Turn right at low request: left side must run faster than right. Repeat left. With P17 at zero, turning the wheel must not propel either side. Record wheel RPM and actual wheel circumference/external gearing before relying on MPH.
...
| Negative P17 in WHEEL / downward controller stick / steering with zero throttle | Both modes reverse; WHEEL does not pivot; CONTROLLER right X commands equal/opposite in-place turning |
