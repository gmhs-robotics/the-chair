...
| Motor temperature | Warn and reduce voltage from 50°C; output reaches zero at 60°C, then recovers as temperature falls | Each child; steering feedback also reduces |
| Drive battery temperature | Trip ≥45°C; start <38°C | Each child and master |
...
| Cumulative duty | 120 s rest reminder; reset by a full 60 s rest; never stops motion | Master HUD |
...
Master motion authority has three internal states: `WaitingForReady`, `Active`, or `Fault(reason)`. Waiting for readiness always commands zero; healthy links, calibrated controls, stopped motors, cool-enough batteries and 500 ms neutral inputs start `Active` automatically. Either B button requests electrical Brake while held; it does not change the active state. Releasing B cannot restore a held throttle request: the chair must stop and all drive controls must remain neutral for 500 ms before motion resumes. Fatal health and control faults latch `Fault` and display **E-STOP**; only the first reason is retained. Releasing drive-enable coasts while retaining `Active`.

Automatic activation requires valid health, each drive motor below 5 RPM, neutral controls for 500 ms, calibration and steering within 8% of center. The master automatically sets the current stationary wheel and P17 positions as zero at startup; the operator must physically center them first. These encoder checks do not prove the chair is physically stationary. The master allows 60 seconds to start and handshake both child Brains; an unresolved missing link then latches E-STOP. After its P21 handshake and first zero command, each child probes one Smart Motor per iteration, repeatedly requests Brake and configuration, and requires all four motors to report valid telemetry without fatal faults continuously for 300 ms. Transient startup motor misses may recover within 5 seconds. A persistent miss latches E-STOP and requires restart after repair. Hot motors remain visible and have reduced voltage; they do not force a reboot. A Brain without a local pack may report either all-zero battery telemetry or valid voltage with current, capacity and temperature all zero; both display as `NO PACK`. Invalid voltage and other partly populated tuples remain battery faults. The master Brain's own battery is not checked.

## What a stop does

A normal operator stop coasts, including zero throttle while drive enable stays held. Either B button requests electrical Brake on both drive sides while held. Release B, stop and hold controls neutral for 500 ms; no restart is needed. A latched system fault retains its first reason, requests Brake and prevents further driving until restart.

Drive motors receive **Coast** for ordinary zero throttle, enable release or L1 hold. B and latched faults request electrical **Brake**. Holding B or the post-B neutral gate removes powered steering feedback; zero throttle while active does **not** remove steering feedback. B sends a zero-voltage Brake command ahead of the normal 40 ms cadence and repeats it while held. Faulted children keep answering health and retrying local Brake. The master retries fault-stop messages to both children every 50 ms. The panic hook attempts local motor Brake before diagnostics; it is not a hardware watchdog.
...
Steering assumes a blue cartridge, reversed motor/sensor direction, encoder-to-wheel ratio 1:1 and ±80° physical travel from a center set automatically at startup or by RECENTER. This makes physical right positive after the installed gear train reports raw right rotation as negative. These are commissioning parameters in `crates/controller/src/geometry.rs` and `crates/controller/src/steering.rs`. Resistive feedback is a limited spring/damper, active only when the chair is ON and neither B nor the post-B neutral hold is active. Position beyond nominal travel clamps the input. Raw encoder velocity is bounded before damping because VEXos can emit single-sample spikes while the motor is back-driven; neither condition alone latches a fault. One to four consecutive invalid samples coast steering feedback and log warnings; a valid sample recovers it, while the fifth consecutive invalid sample latches. Current-related `F2`, `F4`, `F8`, finite measured current and physical obstruction remain diagnostic only because the 0.4 A and 1.2 V limits constrain the motor. Heat warns and reduces feedback power; unknown fault bits and failed output commands still latch. B braking and the neutral hold remove powered feedback and coast the steering motor.
